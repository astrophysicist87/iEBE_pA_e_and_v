...
int main(int argc, char *argv[])
{
	...
	double bin_centers_O[n_bin_centers];
	double bin_centers_S[n_bin_centers];
	double bin_centers_L[n_bin_centers_L];
	const double delta_qo = 0.01;	//bin width of 10 MeV
	const double delta_qs = 0.01;	//bin width of 10 MeV
	const double delta_ql = 0.01;	//bin width of 10 MeV
	linspace(bin_centers_O, -0.05, 0.05, n_bin_centers);
	linspace(bin_centers_S, -0.05, 0.05, n_bin_centers);
	linspace(bin_centers_L, -0.03, 0.03, n_bin_centers_L);
	...
	double * ql_slice_pts = new double [n_bin_centers_L*n_qpts_per_bin];
	double * qo_slice_wts = new double [n_bin_centers*n_qpts_per_bin];
	double * qs_slice_wts = new double [n_bin_centers*n_qpts_per_bin];
	double * ql_slice_wts = new double [n_bin_centers_L*n_qpts_per_bin];
	double * dummy_pts = new double [n_qpts_per_bin];
	double * dummy_wts = new double [n_qpts_per_bin];

	for (int ibo = 0; ibo < n_bin_centers; ++ibo)
	{
		gauss_quadrature(n_qpts_per_bin, 1, 0.0, 0.0, bin_centers_O[ibo] - 0.5*delta_qo, bin_centers_O[ibo] + 0.5*delta_qo, dummy_pts, dummy_wts);
	...
	{
		gauss_quadrature(n_qpts_per_bin, 1, 0.0, 0.0, bin_centers_S[ibs] - 0.5*delta_qs, bin_centers_S[ibs] + 0.5*delta_qs, dummy_pts, dummy_wts);
	...
	for (int ibl = 0; ibl < n_bin_centers_L; ++ibl)
	{
		gauss_quadrature(n_qpts_per_bin, 1, 0.0, 0.0, bin_centers_L[ibl] - 0.5*delta_ql, bin_centers_L[ibl] + 0.5*delta_ql, dummy_pts, dummy_wts);
	...
	for (int ibo = 0; ibo < n_bin_centers; ++ibo)
	{
		...
		for (int iKT = 0; iKT < nKTbins; ++iKT)
		{
			...
			CF_averaged_over_qbin /= delta_qo*delta_qs*delta_ql;

			cout << KTlls[iKT] << "   " << KTuls[iKT] << "   "
				<< bin_centers_O[ibo] << "   " << bin_centers_S[ibs] << "   "
				<< bin_centers_L[ibl] << "   " << CF_averaged_over_qbin << endl;
		}
