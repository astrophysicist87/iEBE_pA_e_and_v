...
//Added by Chris Plumberg to artificially shrink nucleon
const double shrinking_factor = 1.0; // 0.25 corresponds to factor 2 reduction in width
      return shrinking_factor * ( x[i]*pow(srt*srt,eps[i]) + y[i]*pow(srt*srt,-eta[i]) );
}
...
//Added by Chris Plumberg to artificially shrink nucleon
const double shrinking_factor = 1.0; // 0.25 corresponds to factor 2 reduction in width
    return shrinking_factor * facel*sig*sig/bel;
}
